...
%
\fix{Additionally, we derived, visualized, and compared the finite-time Lyapunov exponent (FTLE) scalar field using select Lagrangian$_{Local}$ flow maps for each of these data sets to the ground truth FTLE field.}
...
%
\fix{While Figure~\ref{abc_ftle} shows the FTLE field derived from the reduced Lagrangian$_{Local}$ test T6 is similar to the ground truth, it contains some error along the node boundaries.}
...
%
\fix{Figure~\ref{nyx_ftle} compares the FTLE field derived by interpolating a single longer interval~(T3) to interpolating multiple intervals~(T1) of Lagrangian$_{Local}$ flow maps.
%
...
%
In contrast, the FTLE derived from a single interval of T3 produced a more accurate result. 
}
%
...
%
%The overall structure of FTLE ridges is preserved
...
%
Considering the 1:8 data reduction for T3, the overall FTLE ridge structure can be visualized with a small loss of accuracy.}
%
