...
%
To identify limitations, we evaluated Lagrangian$_{Local}$ in situations where this was not the case, and considered the outcome when the number of processes operating over a fixed grid size increased.
%
...
%
Further, the reconstruction error distribution matched the violin plots of T4, T5, and T6 in Figure~\ref{fig:abc_plot}. 
%
Even for longer storage intervals, the error remained small, and discarded trajectories were reconstructed with high accuracy for the ABC data set.
...
%
Comparing tests T3 and T4 (1:1 data reduction factor) to tests T7 and T8 (1:8 data reduction factor), although T3 and T4 discarded a greater percentage of samples, they remained more accurate due to the overall number of particles used to sample the domain.
%
